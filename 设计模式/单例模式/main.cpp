...
然而现代CPU和编译器高并发下可能会进行乱序重排操作，因而创建对象new CSingleton的第2步可能会晚于第3步进行指令调用，这个时候如果其他线程调用getInstance，发现m_singleton不为空，而直接返回m_singleton，但m_singleton因为还没有构造，导致出现出现未定义的的行为。*/
class Singleton {
private:
    static Singleton *m_singleton;
    static mutex m_mutex;
