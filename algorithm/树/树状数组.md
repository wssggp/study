...
![Binary ranges](..\..\assert\binary-ranges.png)
...
![Compute sum by going up and left based on Binary Indexed Tree](..\..\assert\rang.png)

# 利用Binary Indexed Tree求prefix sum或range sum

通过上面的例子我们得知求前缀和的过程事实上是在树状数组所代表的抽象的树形结构中不断移动寻找上一层母结点并求和的过程。上面例子中树状数组所表示的树如下图所示：

![Tree representation of the Binary Indexed Tree](..\..\assert\tree.png)
