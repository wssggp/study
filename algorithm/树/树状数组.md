...
![Binary ranges](D:\work\notes\study\assert\binary-ranges.png)
...
![Compute sum by going up and left based on Binary Indexed Tree](D:\work\notes\study\assert\rang.png)

# 利用Binary Indexed Tree求prefix sum或range sum

通过上面的例子我们得知求前缀和的过程事实上是在树状数组所代表的抽象的树形结构中不断移动寻找上一层母结点并求和的过程。上面例子中树状数组所表示的树如下图所示：

![Tree representation of the Binary Indexed Tree](D:\work\notes\study\assert\tree.png)
...
![Update number](https://img-blog.csdnimg.cn/20190319155152852.png?x-oss-process=image/watermark,type_ZmFuZ3poZW5naGVpdGk,shadow_10,text_aHR0cHM6Ly9ibG9nLmNzZG4ubmV0L1lhb2thaV9Bc3N1bHRNYXN0ZXI=,size_16,color_FFFFFF,t_70)
